...
def test_cleanup_final_segments_do_not_overlap_when_starts_equal() -> None:
    utterances = [
        DraftUtterance(
            utterance_id=0,
            region_id=0,
    ...
    assert all(left.end <= right.start for left, right in zip(result.final_segments, result.final_segments[1:]))
    assert result.final_segments[1].cleanup_adjusted is True
    assert result.final_segments[1].cleanup_reason == "overlap_shift_current_start"
